...
export default async function CategoryPage({ params }: CategoryPageProps) {
  ...
  // Get sections and menu items for this category
  ...
  const uncategorizedItems = menuItems.filter(
    (item) => !sections.some((s) => s.id === item.sectionId)
  );
  ...
  return (
    <div className="space-y-8">
      <div>
        <Link
          href="/menu"
          className="text-sm text-primary hover:text-secondary mb-4 inline-block"
        >
          ← Back to Menu
        </Link>
                  ...
                  {items.map((item) => (
                    ...
                    >
                      <div className="flex gap-4">
                        <div className="h-16 w-16 rounded-lg border border-border bg-background-alt overflow-hidden shrink-0">
                          {item.imageUrl ? (
                            <img
                              src={item.imageUrl}
                              alt={item.name}
